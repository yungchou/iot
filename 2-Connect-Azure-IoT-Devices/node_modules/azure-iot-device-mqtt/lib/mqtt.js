...
var Mqtt = /** @class */ (function (_super) {
    ...
    function Mqtt(authenticationProvider, mqttBase) {
        ...
        var sdkVersionString = encodeURIComponent('azure-iot-device/' + packageJson.version);
        ...
        if (mqttBase) {
            ...
            _this._mqtt = new azure_iot_mqtt_base_1.MqttBase(sdkVersionString);
        }
        ...
        _this._twinReceiver = new mqtt_twin_receiver_1.MqttTwinReceiver(_this._mqtt);
        _this._fsm = new machina.Fsm({
            ...
            states: {
                disconnected: {
                    ...
                    sendEvent: function (topic, payload, options, sendEventCallback) {
                        ...
                        _this._fsm.handle('connect', function (err) {
                            if (err) {
                                ...
                                _this._fsm.handle('sendEvent', topic, payload, options, sendEventCallback);
                            }
                        });
                    },
                    sendTwinRequest: function (topic, body, sendTwinRequestCallback) {
                        ...
                        _this._fsm.handle('connect', function (err) {
                            if (err) {
                                ...
                                _this._fsm.handle('sendTwinRequest', topic, body, sendTwinRequestCallback);
                            }
                        });
                    },
                    ...
                    getTwinReceiver: function (callback) {
                        _this._fsm.handle('connect', function (err) {
                            if (err) {
                                callback(err);
                            }
                            else {
                                _this._fsm.handle('getTwinReceiver', callback);
                            }
                        });
                    },
                    ...
                    enableTwin: function (callback) {
                        ...
                        _this._fsm.handle('connect', function (err) {
                            if (err) {
                                ...
                                _this._fsm.handle('enableTwin', callback);
                            }
                        });
                    },
                    ...
                    _onEnter: function (connectCallback) {
                        ...
                        _this._authenticationProvider.getDeviceCredentials(function (err, credentials) {
                            if (err) {
                                ...
                                _this._mqtt.connect(credentials, function (err, result) {
                                    debug('connect');
                                    if (err) {
                                        _this._fsm.transition('disconnected', connectCallback, err);
                                    }
                ...
                connected: {
                    ...
                    sendEvent: function (topic, payload, options, sendEventCallback) {
                        _this._mqtt.publish(topic, payload, options, function (err, result) {
                            if (err) {
                    ...
                    sendTwinRequest: function (topic, body, callback) {
                        ...
                        _this._mqtt.publish(topic, body.toString(), { qos: 0, retain: false }, function (err, puback) {
                            if (err) {
                                ...
                                callback(null, new azure_iot_common_1.results.MessageEnqueued(puback));
                            }
                        });
                    },
                    ...
                    getTwinReceiver: function (callback) {
                        ...
                        callback(null, _this._twinReceiver);
                    },
                    enableC2D: function (callback) {
                        _this._setupSubscription(_this._topics.message, callback);
                    },
                    enableMethods: function (callback) {
                        _this._setupSubscription(_this._topics.method, callback);
                    },
                    enableTwin: function (callback) {
                        /*Codes_SRS_NODE_DEVICE_MQTT_16_059: [`enableTwin` shall subscribe to the MQTT topics for twins.]*/
                        _this._twinReceiver.subscribe(callback);
                    },
                    disableC2D: function (callback) {
                    ...
                    disableTwin: function (callback) {
                        ...
                        _this._twinReceiver.unsubscribe(callback);
                    }
                },
                disconnecting: {
    ...
    Mqtt.prototype.sendEvent = function (message, done) {
        ...
        this._fsm.handle('sendEvent', topic, message.data, { qos: 1, retain: false }, function (err, puback) {
            if (err) {
    ...
    Mqtt.prototype.sendTwinRequest = function (method, resource, properties, body, done) {
        ...
        this._fsm.handle('sendTwinRequest', topic, body, function (err, result) {
            if (done)
                done(err, result);
        });
    };
     ...
     * @private
     */
    Mqtt.prototype.enableTwin = function (callback) {
        this._fsm.handle('enableTwin', callback);
    };
    /**
     * @private
     */
    Mqtt.prototype.disableTwin = function (callback) {
    ...
    Mqtt.prototype._configureEndpoints = function (credentials) {
        ...
        credentials.uri = 'mqtts://' + credentials.host;
    ...
    Mqtt.prototype._setupSubscription = function (topic, callback) {
        ...
        this._mqtt.subscribe(topic.name, { qos: 0 }, function (err) {
