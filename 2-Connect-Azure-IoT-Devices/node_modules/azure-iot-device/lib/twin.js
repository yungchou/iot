...
var twin_errors_1 = require("./twin_errors");
/**
...
var Twin = /** @class */ (function (_super) {
    ...
    Twin.prototype._subscribe = function (done) {
        ...
        function handleError(topic) {
            if (!errAlreadySent) {
                errAlreadySent = true;
                cleanupAndReturn(new azure_iot_common_1.errors.ServiceUnavailableError('failed to subscribe to ' + topic));
            }
        }
        ...
        timeout = setTimeout(function () {
            cleanupAndReturn(new azure_iot_common_1.errors.TimeoutError('subscription to twin messages timed out'));
        }, Twin.timeout);
    ...
    Twin.prototype._sendTwinRequest = function (method, resource, properties, body, done) {
        ...
        var timeout = null;
        function cleanupAndReturn(err, body) {
            clearTimeout(timeout);
            timeout = null;
        ...
        function handleResponse(resp) {
            ...
            if (resp.$rid.toString() === propCopy.$rid.toString()) {
                ...
                if (resp.status.toString() === '200' || resp.status.toString() === '204') {
                    ...
                    cleanupAndReturn(twin_errors_1.translateError(resp, resp.status));
                }
            }
        }
        ...
        timeout = setTimeout(function () {
            cleanupAndReturn(new azure_iot_common_1.errors.TimeoutError('message timed out'), null);
        }, Twin.timeout);
